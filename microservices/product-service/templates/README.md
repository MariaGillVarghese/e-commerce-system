...
This folder contains Kubernetes manifests for deploying the Product Service.
...
- `ecommerce-product-service:latest` Docker image pushed to your registry
...
```bash
helm install product-service . -n ecommerce
```
...
# Check service
kubectl get service product-service -n ecommerce
...
```bash
kubectl get configmap product-service-config -n ecommerce
kubectl describe configmap product-service-config -n ecommerce
```
